...
We denote the set of paths of a graph $G$ with $\pathcat{G}$. $\pathcat{G}$ is called the 
...
\transrem{The letter $\mathcal{W}$ comes from the German word ''Wegekategorie''
(path category).}
...
$G_2 = (V_2, E_2)$ consists of a single point $v_0$ and two loops labeled with
...
Now we define for paths $(v, e_1, \ldots, e_n, v') \in \pathcat{G_1}$:
\[ \phi_2((v, e_1, \ldots, e_n, v')) := (v_0, \phi'_2(e_1), \ldots,
\phi'_2(e_n), v_0) \].

Obviously $\phi = (\pathcat{G_1}, \pathcat{G_2}, \phi1, \phi_2)$ is a functor.

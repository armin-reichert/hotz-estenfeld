...
(\pathcategory{G}, X^*,\alpha_1, \alpha_2)$ a functor with $\alpha_1 : V \to \{ X^*
\},\ \alpha_2 : \pathcategory{G} \to X^*$.
...
If we define 
\[ \pathcategory{G}(S, F) := \{ w \in \pathcategory{G}\ |\ Q(w) \in S \wedge Z(w)
\in F \}, \]
then \[ L_{\fa{A}} := \alpha_2(\pathcategory{G}(S, F)) \] 
is called the set {\bf accepted by} the automaton. 
\end{definition}

We will also write shortly $\alpha$ instead of $\alpha_2$ and $\pathcategory{}(S,F)$
instead of $\pathcategory{G}(S,F)$.
...
\begin{enumerate}
  ...
  e'_1, \ldots, e'_k, Z(e)) \in \pathcategory{G'}$ and define a new graph $G' =
...
\alpha(\pathcategory{W}(S_{\fa{A}}, P))$.
...
\begin{enumerate}
  ...
  By construction of graph $G$ there exists a path in $\pathcategory{G}(S1, F2)$ of
  the form
  ...
  an accepting path in $\pathcategory{G}(S, F)$. Then there exist states $P_i \in
...
\phi(\alpha_2(\pathcategory{G}(S, F))) = \beta_2(\pathcategory{G}(S,F))\]
...
\begin{lemma}
Set $G = (V, E)$. Then $\pathcategory{G}(S,F) \in REG(E^*)$.
\end{lemma}
...
Let $G = (V, E)$ be a graph. Then $\pathcategory{G}(S, F)$ is local over $E$.
\end{lemma}

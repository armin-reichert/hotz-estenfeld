...
points of the graph become points in $\mathbb{R}^3$, the edges become connecting
lines in $\mathbb{R}^3$ which do not pairwise intersect.
...
(vertices), a set $E$ of edges and a mapping $\rho: E \to Pot(V)$ with
$card(\rho(e)) <= 2$ for $e \in E$. $\rho(e)$ is the set of {\em border points} of $e$.
\end{definition}
...
We denote the set of paths of a graph $G$ with $\pathcategory{G}$. $\pathcategory{G}$ is 
...
\begin{enumerate}
  ...
  morphisms $(A, A, \{(a, a) \mid a\in A \})$ are the units for each set $A \in O$.
...
Consider the path categories $\pathcategory{G_1}$ and $\pathcategory{G_2}$. For $v
...
Now we define for the paths $(v, e_1, \ldots, e_n, v') \in \pathcategory{G_1}$:
\[ \phi_2((v, e_1, \ldots, e_n, v')) := (v_0, \phi'_2(e_1), \ldots,
\phi'_2(e_n), v_0) \]

Obviously $\phi = (\pathcategory{G_1}, \pathcategory{G_2}, \phi1, \phi_2)$ is a
...
$\phi = (\pathcategory{G_1}, \pathcategory{G_2}, \phi_1, \phi_2)$ is a functor.
...
Consider $\pathcategory{G}$, the path category of $G$, and $MAT(\mathbb{N})$,
...
to the set $\setof{e \in E_1 \mid Q(e) = v}$ and $\setof{e' \in E_2 \mid Q(e') =
\phi_1(v)}$ and to $\setof{e \in E_1 \mid Z(e) = v}$ and $\setof{e' \in E_2 \mid
Z(e') = \phi_1(v)}$ for $v \in V_1$ is bijective.
\end{definition}

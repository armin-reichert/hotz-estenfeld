...
If we define 
\[ \pathcat{G}(S, F) := \setof{w \in \pathcat{G} \mid Q(w) \in S,\ Z(w) \in F}
\] 
then
...
\begin{definition}[set of regular languages over free monoid]
Let $X$ be an alphabet. 
\[ REG(X^*) := \setof{L \subset X^* \mid \mbox{ there exists a finite automaton
}\fa{A} \mbox{ with } L = \lang{A}}
\]
$REG(X^*)$ is the {\bf set of regular languages} over the free monoid $X^*$.
\end{definition}
...
\[ \delta = \setof{(a, P_1, P_2) \in X \times V \times V) \mid 
...
Example 1: Let $X = \setof{a, b}$ and $L = \setof{(a b)^{2n} \mid n \in
\mathbb{N}}$.
It holds: $L \in REG(\setof{a, b}^*)$.

The following acceptor accepts $L$ (exercise):

$\fa{A} = (G, \setof{a, b}, {1}, {1}, \alpha)$.

\begin{center}
\input{figures/chapter2/chapter2-1-example1.tikz}
\end{center}
...
\begin{sideways}
\input{figures/chapter2/chapter2-1-example2.tikz}
\end{sideways}
\end{minipage}
...
\]
\[ \phi(L) := \setof{\phi(w) \mid w \in L} \in REG(X^*) \]
\end{theorem}
...
\begin{lemma}
For $x \in X$ it holds $\setof{x} \in REG(X^*)$ and $\emptyset \in REG(X^*)$.
\end{lemma}

Proof: Set $G = (\setof{S,F}, \setof{e})$ be a graph, then the finite acceptor
\[\fa{A}_x = (G, \setof{x}, \setof{S}, \setof{F}, \alpha)\] with $\alpha_2(e) =
x$ fulfills $\lang{A} = \setof{x}$.

Set $G = (\setof{S,F}, \emptyset)$, then $\fa{A} = (G, \emptyset, \setof{S},
...
Let $S, F \subset X$ and $R \subset X \times X$ be a relation on $X$ with 
\[ L = \setof{x_1 \cdots x_k \mid x_1 \in S, x_k \in F, (x_i, x_{i+1}) \in R,\ i
= 1, \ldots, k - 1} \]
\end{definition}
...
$REG(X_\infty^*)$ is closed under $\phi$.
	...
	over $X_\infty^*$.
	\item $REG(X_\infty^*)$	 contains the sets $\setof{x}, x \in X_\infty^*$ and

...
\[ L \in \reglang(X^*), \phi : X^* \to Y^* \mbox{ monoid homomorphism } \Rightarrow
\]
...
\begin{lemma}
Set $G = (V, E)$. Then $\pathcat{G}(S,F) \in \reglang(E^*)$.
\end{lemma}
...
$L \in \reglang(X^*)$, then there exists a local set $R$ over $X$ and a monoid
...
\[ \reglang(X_\infty^*) = \bigcup_{X	\subset X_\infty} \reglang(X^*), \mbox{$X$ is finite}
\]
...
\begin{maintheorem}
\begin{enumerate}
  \item $\reglang(X_\infty^*)$ is a Boolean Algebra with operations union, intersection and
complement. 
